...
function limitedRepeat() {
  var logHi = setInterval(() => console.log("hi"), 1000);
  var stopLogHi = setTimeout(() => clearInterval(logHi), 5035);
}
...
function everyXsecsForYsecs(callback, interval, duration) {
  var callFuncOnInterval = setInterval(callback, interval);
  var stopCallFuncOnInterval = setTimeout(() => clearInterval(callFuncOnInterval), duration);
}
...
function debounce(callback, interval) {
  var invocationTimes = [],
    logCallback = {
      differenceBetweenInvocations: 0,
      logStatus: false,
    };

  function inner() {
    invocationTimes.push(new Date().getTime());

    if (invocationTimes.length < 2) invocationTimes[1] = invocationTimes[0];
    if (invocationTimes.length > 2) invocationTimes.shift();
    if (invocationTimes.length === 2) {
      logCallback.differenceBetweenInvocations = invocationTimes[1] - invocationTimes[0];
    }

    function shouldLog() {
      if (logCallback.differenceBetweenInvocations === 0 || logCallback.differenceBetweenInvocations >= 3000) {
        logCallback.logStatus = true;
      } else logCallback.logStatus = false;
    }

    shouldLog();
    if (logCallback.logStatus) return callback();
