...
// console.log(addByTwo(1), addByTwo(2),addByTwo(3),addByThree(1),addByThree(2),addByFour(4),addByFour(5));
...
// const funcDelay = () => console.log(5);
// delay(funcDelay, 2000)
...
function rollCall(names) {
  ...
  function log() {
    index++;
    return console.log(names[index] === undefined ? "Everyone accounted for" : names[index]);
  }
  return log;
}
...
// CHALLENGE 9
// Create a function cycleIterCreate a function saveOutput that accepts a function (that will accept one argument), and a string (that will act as a password). saveOutput will then return a function that behaves exactly like the passed-in function, except for when the password string is passed in as an argument. When this happens, the returned function will return an object with all previously passed-in arguments as keys, and the corresponding outputs as values.ator that accepts an array, and returns a function. The returned function will accept zero arguments. When first invoked, the returned function will return the first element of the array. When invoked a second time, the returned function will return the second element of the array, and so forth. After returning the last element of the array, the next invocation will return the first element of the array again, and continue on with the second after that, and so forth.
...
function censor() {
  var strings = [];
  function string(a, b) {
    if (b !== undefined) strings.push([a, b]);

    if (strings.length == 2) {
      var oldWords = strings[0];
      var newWords = strings[1];

      if (a.includes(oldWords[0])) a.replace(oldWords[0], newWords[0]);
      if (a.includes(oldWords[1])) a.replace(oldWords[1], newWords[1]);
    }
    return a;
  }
  return string;
...
function average() {
  var sum = 0,
    count = 0;
  function inner(n) {
    if (n === undefined) return sum / count || 0;
    sum += n;
    count++;
    return sum / count;
  }
...
function makeFuncTester(arrOfTests) {
  function inner(callback) {
    return arrOfTests.map((subArray) => callback(subArray[0]) === subArray[1]).reduce((acc, curr) => acc && curr, true);
  }
  return inner;
}
